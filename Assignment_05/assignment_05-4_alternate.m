...
%DOES converge faster than AB4 for N = 20. In fact, for N =20, THE ERROR IS IN ORDER 10^-4, which is good! <3
...
%Our error for N=50 is now smaller than the magnitude as the actual solution
%So relative error would still be attrocious.
%AM4 DOES seems to converge even faster than AB4, especially for small N!

%Postulate: AM4 i sa stable scheme to solve these problems for any N.
...
%Same OG points from RK4? So why did I do AB4?
	for (i = 5:1:(N+1)) %%Iterate, since w(1):w(4) are filled, start at 5
        ...
        w(i)=solve(ode,k); %Ok, this seems a little odd so use solve in a numerics class.
        if(gdb|0)
			fprintf("\n%d\n", w(i))
        end
