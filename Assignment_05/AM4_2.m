...
w(3) = y(3);
%w(4) = y(4);
%Same OG points from RK4.
	for (i = 4:1:(N+1)) %%Iterate, since y(1):y(4) are filled, start at 5

        %Overwrite all points from 3 to N+1!
        w(i) = w(i-1) + h./24.*(9.*f(t(i),y(i)) +19.*f(t(i-1),w(i-1)) -5.*f(t(i-2), w(i-2))  + 1.*f(t(i-3),w(i-3)));%Modus operandi of AB4
					  %Spaghetti coefficient play,
					  %Maybe we can seperate this addition into multiple lines later for a cleaner look?
		if(gdb|0)
			fprintf("\n%d\n", w(i))
